                ...
                <div className='max-w-285 h-full mx-auto grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6'>
                    {productsData.map((product) => (
                        <div key={product.id} className='group'>
                            <div className='relative bg-[#F5F5F5] w-full aspect-square hover:bg-[#0000004D] hover:scale-[1.05] transition-transform duration-200 overflow-hidden'>
                                <div className='absolute top-0 left-0 right-0 p-3 sm:p-4 flex justify-between z-10'>
                                    <div className='bg-[#FFFFFF] px-3 sm:px-4 py-2 rounded-[88px] flex justify-center items-center text-xs sm:text-sm md:text-base'>{product.price}</div>
                                    <LikeButton />
                                </div>
                                <div className='absolute inset-0 flex items-center justify-center bg-black/0 group-hover:bg-black/20 transition-all duration-200 opacity-0 group-hover:opacity-100'>
                                    <button className='cursor-pointer active:bg-[#007a70] bg-[#01C6B5] px-6 sm:px-8 py-3 sm:py-3.5 text-sm sm:text-base font-medium text-white rounded hover:opacity-90 transition'>Add to cart</button>
                                </div>
                                <img src={product.image} alt={product.name} className='w-full h-full object-cover' />
                            </div>
                            <h1 className='text-center mt-4 sm:mt-6 font-normal text-lg sm:text-xl md:text-[24px] leading-snug'>{product.name}</h1>
                        </div>
                    ))}
                </div>
            </div>
